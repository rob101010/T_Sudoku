"""
Function derived from multiple_choice>_to_excel, ignore printing multiple candidates
...
def multiple_choice():
    ...
    for i in range(9):
        for j in range(9):
            ...
            column_row_filled = []
            box_filled = []
            if pd.isnull(df_temp.iloc[i, j]):
                column_filled = df_temp.iloc[:, j]
                row_filled = df_temp.iloc[i, :]
                if i // 3 == 0:
                    r_low = 0
                    r_high = 3
                if i // 3 == 1:
                ...
                for row_box in range(r_low, r_high):
                    for column_box in range(c_low, c_high):
                        box_filled = box_filled + [df_temp.iloc[row_box, column_box]]
            ...
            if pd.isnull(df_temp.iloc[i, j]):
                if len(missing_numbers) > 0:
                    df_temp.iloc[i, j] = missing_numbers_int
