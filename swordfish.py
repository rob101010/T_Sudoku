# ONLY IN SWORDFISH, USER CHOICE TO USE FUNCTION: MULTIPLE CHOICE
import pandas as pd
import numpy as np
import sys
...
for q in range(1, 10):
    ...
    # start with identifying ROWS: check if there are 3 ROWS, where check_number is in max 3 cells (often 2 cells per row)
    # Then see if these cells are in 3 COLUMNS ==> ID SO : SWORDFISH
    # ACTION: ELIMINATE all check-number in COLUMNS that are not in 3 identified rows

    # Check if within a ROW, check_number_1 is present 2 or 3 times.
    # if so, add COLUMN index for that row to swordfish_row list
    # So swordfish_row is list of list, where each list within list contains COLUMN index if of 2 or 3 entries in that row
    # to later identify ROW number in case of Swordfish, row numbers are added to [record row number] in case 2 or 3 entries

    swordfish_row = []
    record_row_number = []
    if len(col_index_row_1) == 1 or len(col_index_row_1) == 2 or len(col_index_row_1) == 3:
        swordfish_row.append(col_index_row_1)
        record_row_number.append('row 1')
    if len(col_index_row_2) == 1 or len(col_index_row_2) == 2 or len(col_index_row_2) == 3:
        swordfish_row.append(col_index_row_2)
        record_row_number.append('row 2')
    if len(col_index_row_3) == 1 or len(col_index_row_3) == 2 or len(col_index_row_3) == 3:
        swordfish_row.append(col_index_row_3)
        record_row_number.append('row 3')
    if len(col_index_row_4) == 1 or len(col_index_row_4) == 2 or len(col_index_row_4) == 3:
        swordfish_row.append(col_index_row_4)
        record_row_number.append('row 4')
    if len(col_index_row_5) == 1 or len(col_index_row_5) == 2 or len(col_index_row_5) == 3:
        swordfish_row.append(col_index_row_5)
        record_row_number.append('row 5')
    if len(col_index_row_6) == 1 or len(col_index_row_6) == 2 or len(col_index_row_6) == 3:
        swordfish_row.append(col_index_row_6)
        record_row_number.append('row 6')
    if len(col_index_row_7) == 1 or len(col_index_row_7) == 2 or len(col_index_row_7) == 3:
        swordfish_row.append(col_index_row_7)
        record_row_number.append('row 7')
    if len(col_index_row_8) == 1 or len(col_index_row_8) == 2 or len(col_index_row_8) == 3:
        swordfish_row.append(col_index_row_8)
        record_row_number.append('row 8')
    if len(col_index_row_9) == 1 or len(col_index_row_9) == 2 or len(col_index_row_9) == 3:
        swordfish_row.append(col_index_row_9)
        record_row_number.append('row 9')

    # List [swordfish_row] contains list of column index for each row with 2 or 3 entries for check_number
    # List [swordfish_row_comb_3] is temporary list of all possible combinations of 3 rows.
    # ==> use 3 times a for loop to create all possible combinations of 3 cells
    # This temporary list is used to check if Swordfish: if not true, create new tem list, if Yes ==> quit program
    for i in range(len(swordfish_row)):
        for j in range(i + 1, len(swordfish_row)):
            for k in range(j + 1, len(swordfish_row)):
                swordfish_row_comb_3 = [swordfish_row[i]]
                swordfish_row_comb_3 = swordfish_row_comb_3 + [swordfish_row[j]]
                swordfish_row_comb_3 = swordfish_row_comb_3 + [swordfish_row[k]]
                swordfish_flat = [item for sublist in swordfish_row_comb_3 for item in sublist]
                ...
                if swordfish_number == 3:
                    ...
                    columns_with_swordfish = []
                    for c in swordfish_flat:
                        if c not in columns_with_swordfish:
    ...
    # start with identifying COLUMNS: check if there are 3 COLUMNS, where check_number is in max 3 cells (often just 2)
    # Then see if these cells are in 3 ROWS ==> ID SO : SWORDFISH
    # ACTION: ELIMINATE all check-number in ROWS that are not in 3 identified columns


    # SWORDFISH_columns: more detail
    # Check if within a COLUMN, check_number_1 is present 2 or 3 times.
    # if so, add ROW index for that column to swordfish_column list
    # So swordfish_column is list of list, where each list within list contains ROW index if of 2 / 3 entries in that column
    # to later identify COLUMN number if Swordfish, column numbers are added to [record column number] if 2/3 entries


    swordfish_column = []
    record_column_number = []
    if len(row_index_column_1) == 1 or len(row_index_column_1) == 2 or len(row_index_column_1) == 3:
        swordfish_column.append(row_index_column_1)
        record_column_number.append('column A')
    if len(row_index_column_2) == 1 or len(row_index_column_2) == 2 or len(row_index_column_2) == 3:
        swordfish_column.append(row_index_column_2)
        record_column_number.append('column B')
    if len(row_index_column_3) == 1 or len(row_index_column_3) == 2 or len(row_index_column_3) == 3:
        swordfish_column.append(row_index_column_3)
        record_column_number.append('column C')
    if len(row_index_column_4) == 1 or len(row_index_column_4) == 2 or len(row_index_column_4) == 3:
        swordfish_column.append(row_index_column_4)
        record_column_number.append('column D')
    if len(row_index_column_5) == 1 or len(row_index_column_5) == 2 or len(row_index_column_5) == 3:
        swordfish_column.append(row_index_column_5)
        record_column_number.append('column E')
    if len(row_index_column_6) == 1 or len(row_index_column_6) == 2 or len(row_index_column_6) == 3:
        swordfish_column.append(row_index_column_6)
        record_column_number.append('column F')
    if len(row_index_column_7) == 1 or len(row_index_column_7) == 2 or len(row_index_column_7) == 3:
        swordfish_column.append(row_index_column_7)
        record_column_number.append('column G')
    if len(row_index_column_8) == 1 or len(row_index_column_8) == 2 or len(row_index_column_8) == 3:
        swordfish_column.append(row_index_column_8)
        record_column_number.append('column H')
    if len(row_index_column_9) == 1 or len(row_index_column_9) == 2 or len(row_index_column_9) == 3:
        swordfish_column.append(row_index_column_9)
        record_column_number.append('column I')

    # List [swordfish_column] contains list of row index for each column with 2 or 3 entries for check_number
    # List [swordfish_column_comb_3] is temporary list of all possible combinations of 3 columns
    # This temporary list is used to check if Swordfish: if not true, create new tem list,
    for i in range(len(swordfish_column)):
        for j in range(i + 1, len(swordfish_column)):
            for k in range(j + 1, len(swordfish_column)):
                swordfish_column_comb_3 = [swordfish_column[i]]
                swordfish_column_comb_3 = swordfish_column_comb_3 + [swordfish_column[j]]
                swordfish_column_comb_3 = swordfish_column_comb_3 + [swordfish_column[k]]
                swordfish_column_flat = [item for sublist in swordfish_column_comb_3 for item in sublist]
                ...
                if swordfish_column_number == 3:
                    ...
                    rows_with_swordfish = []
                    for c in swordfish_column_flat:
                        if c not in rows_with_swordfish:
