...
for q in range(1, 10):     # (6, 7) (1, 10)
    ...
    # start with identifying ROWS: check if there are 4 ROWS, where check_number is in max 4 cells (often 2/3 per row)
    # Then see if these cells are in 4 COLUMNS ==> ID SO : JELLYFISH
    # ACTION: ELIMINATE all check-number in COLUMNS that are not in 4 identified rows

    # Check if within a ROW, check_number_1 is present 2, 3 or 4 times.
    # if so, add COLUMN index for that row to squirmbag_row list
    # So squirmbag_row is list of list, where each list within list contains COLUMN index if 2,3,4 entries in that row
    # to later identify ROW number in case of Squirmbag, row numbers are added to [record row number] if 2,3,4 entries

    jellyfish_row = []
    record_row_number = []
    if len(col_index_row_1) == 1 or len(col_index_row_1) == 2 or len(col_index_row_1) == 3 or len(col_index_row_1) == 4:
        jellyfish_row.append(col_index_row_1)
        record_row_number.append('row 1')
    if len(col_index_row_2) == 1 or len(col_index_row_2) == 2 or len(col_index_row_2) == 3 or len(col_index_row_2) == 4:
        jellyfish_row.append(col_index_row_2)
        record_row_number.append('row 2')
    if len(col_index_row_3) == 1 or len(col_index_row_3) == 2 or len(col_index_row_3) == 3 or len(col_index_row_3) == 4:
        jellyfish_row.append(col_index_row_3)
        record_row_number.append('row 3')
    if len(col_index_row_4) == 1 or len(col_index_row_4) == 2 or len(col_index_row_4) == 3 or len(col_index_row_4) == 4:
        jellyfish_row.append(col_index_row_4)
        record_row_number.append('row 4')
    if len(col_index_row_5) == 1 or len(col_index_row_5) == 2 or len(col_index_row_5) == 3 or len(col_index_row_5) == 4:
        jellyfish_row.append(col_index_row_5)
        record_row_number.append('row 5')
    if len(col_index_row_6) == 1 or len(col_index_row_6) == 2 or len(col_index_row_6) == 3 or len(col_index_row_6) == 4:
        jellyfish_row.append(col_index_row_6)
        record_row_number.append('row 6')
    if len(col_index_row_7) == 1 or len(col_index_row_7) == 2 or len(col_index_row_7) == 3 or len(col_index_row_7) == 4:
        jellyfish_row.append(col_index_row_7)
        record_row_number.append('row 7')
    if len(col_index_row_8) == 1 or len(col_index_row_8) == 2 or len(col_index_row_8) == 3 or len(col_index_row_8) == 4:
        jellyfish_row.append(col_index_row_8)
        record_row_number.append('row 8')
    if len(col_index_row_9) == 1 or len(col_index_row_9) == 2 or len(col_index_row_9) == 3 or len(col_index_row_9) == 4:
        jellyfish_row.append(col_index_row_9)
        record_row_number.append('row 9')

    # List [jellyfish_row] contains list of column index for each row with 2, 3, 4 entries for check_number
    # List [jelly_row_comb_4] is temporary list of all possible combinations of 4 rows
    # This temporary list is used to check if Swordfish: if not true, create new tem list
    for i in range(len(jellyfish_row)):
        for j in range(i + 1, len(jellyfish_row)):
            for k in range(j + 1, len(jellyfish_row)):
                for l in range(k + 1, len(jellyfish_row)):
                    jellyfish_row_comb_4 = [jellyfish_row[i]]
                    jellyfish_row_comb_4 = jellyfish_row_comb_4 + [jellyfish_row[j]]
                    jellyfish_row_comb_4 = jellyfish_row_comb_4 + [jellyfish_row[k]]
                    jellyfish_row_comb_4 = jellyfish_row_comb_4 + [jellyfish_row[l]]
                    jellyfish_flat = [item for sublist in jellyfish_row_comb_4 for item in sublist]
                    ...
                    if jellyfish_number == 4:
                        ...
                        columns_with_jellyfish = []
                        for c in jellyfish_flat:
                            if c not in columns_with_jellyfish:
                                columns_with_jellyfish.append(c)

                        columns_with_jellyfish.sort()

                        print('Eliminate (in COLUMNS) number ' + check_number + ' in other rows in the columns:')
                        for a in range(len(columns_with_jellyfish)):
                            print(convert_to_column_letter[columns_with_jellyfish[a]])
    ...
    # start with identifying COLUMNS: check if there are 4 COLUMNS, where check_number is in max 4 cells(often just 2,3)
    # Then see if these cells are in 4 ROWS ==> ID SO : JELLYFISH
    # ACTION: ELIMINATE all check-number in ROWS that are not in 4 identified columns

    # JELLYFISH_columns: more detail
    # Check if within a COLUMN, check_number_1 is present 2, 3 or 4 times.
    # if so, add ROW index for that column to squirmbag_column list
    # So jellyfish_column is list of list, where each list within list contains ROW index if 2,3,4 entries in column
    # to later identify COLUMN number if Jellyfish, column numbers are added to [record column number] if 2/3/4 entries


    jellyfish_column = []
    record_column_number = []
    if len(row_index_column_1) == 1 or len(row_index_column_1) == 2 or len(row_index_column_1) == 3 or len(row_index_column_1) == 4:
        jellyfish_column.append(row_index_column_1)
        record_column_number.append('column A')
    if len(row_index_column_2) == 1 or len(row_index_column_2) == 2 or len(row_index_column_2) == 3 or len(row_index_column_2) == 4:
        jellyfish_column.append(row_index_column_2)
        record_column_number.append('column B')
    if len(row_index_column_3) == 1 or len(row_index_column_3) == 2 or len(row_index_column_3) == 3 or len(row_index_column_3) == 4:
        jellyfish_column.append(row_index_column_3)
        record_column_number.append('column C')
    if len(row_index_column_4) == 1 or len(row_index_column_4) == 2 or len(row_index_column_4) == 3 or len(row_index_column_4) == 4:
        jellyfish_column.append(row_index_column_4)
        record_column_number.append('column D')
    if len(row_index_column_5) == 1 or len(row_index_column_5) == 2 or len(row_index_column_5) == 3 or len(row_index_column_5) == 4:
        jellyfish_column.append(row_index_column_5)
        record_column_number.append('column E')
    if len(row_index_column_6) == 1 or len(row_index_column_6) == 2 or len(row_index_column_6) == 3 or len(row_index_column_6) == 4:
        jellyfish_column.append(row_index_column_6)
        record_column_number.append('column F')
    if len(row_index_column_7) == 1 or len(row_index_column_7) == 2 or len(row_index_column_7) == 3 or len(row_index_column_7) == 4:
        jellyfish_column.append(row_index_column_7)
        record_column_number.append('column G')
    if len(row_index_column_8) == 1 or len(row_index_column_8) == 2 or len(row_index_column_8) == 3 or len(row_index_column_8) == 4:
        jellyfish_column.append(row_index_column_8)
        record_column_number.append('column H')
    if len(row_index_column_9) == 1 or len(row_index_column_9) == 2 or len(row_index_column_9) == 3 or len(row_index_column_9) == 4:
        jellyfish_column.append(row_index_column_9)
        record_column_number.append('column I')

    # List [jellyfish_column] contains list of row index for each column with 2 or 3 entries for check_number
    # List [jellyfish_column_comb_4] is temporary list of all possible combinations of 3 columns
    # This temporary list is used to check if Swordfish: if not true, create new tem list,
    for i in range(len(jellyfish_column)):
        for j in range(i + 1, len(jellyfish_column)):
            for k in range(j + 1, len(jellyfish_column)):
                for l in range(k + 1, len(jellyfish_column)):
                    jellyfish_column_comb_4 = [jellyfish_column[i]]
                    jellyfish_column_comb_4 = jellyfish_column_comb_4 + [jellyfish_column[j]]
                    jellyfish_column_comb_4 = jellyfish_column_comb_4 + [jellyfish_column[k]]
                    jellyfish_column_comb_4 = jellyfish_column_comb_4 + [jellyfish_column[l]]
                    jellyfish_column_flat = [item for sublist in jellyfish_column_comb_4 for item in sublist]
                    ...
                    if jellyfish_column_number == 4:
                        ...
                        rows_with_jellyfish = []
                        for c in jellyfish_column_flat:
                            if c not in rows_with_jellyfish:
                                rows_with_jellyfish.append(c)

                        rows_with_jellyfish.sort()
                        print('Eliminate (in ROWS) number ' + check_number + ' in other columns in the rows:')
                        print(rows_with_jellyfish)
                        print('\n')
