...
#  START ON XYZ WING
        ...
        if len(buddy_series_unique.iloc[0]) == 3:
            ...
            for potential_Z in range(1, 21):
                if len(buddy_series_unique.iloc[potential_Z]) == 2:   # check _cells has 2 entries
                    if (first_number in buddy_series_unique.iloc[potential_Z]) and (second_number in buddy_series_unique.iloc[potential_Z])\
                            or (first_number in buddy_series_unique.iloc[potential_Z]) and (third_number in buddy_series_unique.iloc[potential_Z])\
                            or (second_number in buddy_series_unique.iloc[potential_Z]) and (third_number in buddy_series_unique.iloc[potential_Z]):
                        # make copy of buddy_series_unique, so buddy_s_u not changed if you remove X and Y
                        Z_temp = buddy_series_unique.iloc[potential_Z].copy()
                        ...
                        if check_number_1 + check_number_2 <= number_candidates_XYZ - 1:
                            if Z_list[check_number_1] != Z_list[check_number_1 + check_number_2]:
                                ...
                                if (Z_list[0][0]) in Z_list[1]:
                                    print("\nZYZ wing discovered")
                                    print('XYZ index is ' + str(XYZ_index) + ', cell content ' + str(list(first_number + second_number + third_number)))
